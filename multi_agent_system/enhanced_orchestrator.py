"""Enhanced Multi-Agent Orchestrator for coordinating AI agents with autonomous coding capabilities."""
...
from .enhanced_execution_agent import EnhancedExecutionAgent
...
class EnhancedMultiAgentOrchestrator:
    """Enhanced orchestrator that coordinates all agents and manages task execution with autonomous coding capabilities."""
    
    def __init__(self):
        self.logger = SystemLogger("enhanced_orchestrator")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.logger.info("Initializing enhanced multi-agent orchestrator")
            ...
            self.agents[AgentType.EXECUTION] = EnhancedExecutionAgent()
            
            # Set orchestrator reference in execution agent
            if AgentType.EXECUTION in self.agents:
                self.agents[AgentType.EXECUTION].orchestrator = self
            
            # Initialize agent states
            for agent_type in self.agents.keys():
                self.agent_states[agent_type] = "idle"
            
            self.logger.info("Enhanced orchestrator initialized successfully", agent_count=len(self.agents))
            return True
            
        except Exception as e:
            self.logger.error("Failed to initialize enhanced orchestrator", error=str(e))
            return False
    ...
    def _select_agent_for_task(self, task: Task) -> AgentType:
        ...
        # Special case: if the task is about generating a complete application, use EnhancedExecutionAgent
        if any(keyword in task.description.lower() for keyword in ["create app", "build application", "generate application", "make app"]):
            return AgentType.EXECUTION
    ...
    async def _cross_validate_responses(self, responses: List[Response], original_task: Task) -> Response:
        ...
        try:
            ...
            self.logger.warning("Cross-validation failed, using highest confidence response", error=str(e))
            return max(responses, key=lambda r: r.confidence_score)
    ...
    async def generate_autonomous_application(self, task: Task) -> OrchestrationResult:
        """Generate a complete application autonomously using the enhanced execution agent."""
        ...
        try:
            # Use the enhanced execution agent for autonomous application generation
            execution_agent = self.agents[AgentType.EXECUTION]
            
            # Prepare context for autonomous generation
            context = {
                "is_autonomous_app": True,
                "project_requirements": task.requirements,
                "project_description": task.description
            }
            
            # Generate the application
            response = await execution_agent.generate_application(task, context)
            
            # Store results in memory
            await self._store_knowledge_artifacts(response, task)
            ...
            result = OrchestrationResult(
                ...
                metadata={"autonomous_generation": True}
            )
            
            task.update_status(TaskStatus.COMPLETED)
            self.logger.info("Autonomous application generation completed", task_id=task.id, execution_time=execution_time)
            
            return result
            
        except Exception as e:
            task.update_status(TaskStatus.FAILED)
            self.logger.error("Autonomous application generation failed", task_id=task.id, error=str(e))
            
            # Create error response
            error_response = Response(
                task_id=task.id,
                agent_type=AgentType.EXECUTION,
                content=f"Autonomous application generation failed: {str(e)}",
                confidence_score=0.0,
                execution_time=time.time() - start_time,
                success=False,
                error_message=str(e)
            ...
            return OrchestrationResult(
                ...
                confidence_score=0.0,
                artifacts=[],
                metadata={"error": str(e), "autonomous_generation": True}
            )